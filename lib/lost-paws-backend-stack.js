...
class LostPawsBackendStack extends Stack {
  constructor(scope, id, props) {
    ...
    petTable.addGlobalSecondaryIndex({
      indexName: config_pet_dynamodb.INDEX_KEY,
      partitionKey: { name: config_pet_dynamodb.PARTITION_INDEX_KEY, type: dynamodb.AttributeType.STRING },
    ...
    */
    ...
    const utilsLayer = new lambda.LayerVersion(this, "UtilsLayer", {
      code: lambda.Code.fromAsset(
        path.join(__dirname, "../layers/utils")
      ),
    ...
    const updateLostPetStatusLambda = new lambda.Function(this, 'updateLostPetStatus', {
      runtime: lambda.Runtime.NODEJS_18_X,
    ...
    lost.addMethod('PUT', new apigateway.LambdaIntegration(updateLostPetStatusLambda), {
      apiKeyRequired: true,
    });
