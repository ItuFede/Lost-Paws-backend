...
const httpErrorWrapper = ({ handler, httpMethod, contentType }) => {
  console.log(
    "handler, httpMethod, contentType>>> ",
    handler,
    httpMethod,
    contentType
  );
  ...
  const handler_wrapped = async (event, context) => {
    ...
    try {
      ...
      response.body =
        customHeaders && customHeaders["Content-Type"] === "application/pdf"
          ? body
          : JSON.stringify(body);
      ...
      if (
        ex.name == "UnrecognizedClientException" ||
        ex.name == "ExpiredTokenException" ||
        ex.name == "AccessDeniedException"
      )
